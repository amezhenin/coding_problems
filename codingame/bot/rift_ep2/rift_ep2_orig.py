...
class Zone:
    ...
    def update_pods(self, owner_id, my_pods, enemy, visible, platinum, round):
    ...
    def is_uptodate(self, round):
        return self.explored and self.explored > round - EXPLORE_ROUNDS
...
class Game:
    ...
    def next_round(self):
        ...
        for i in range(self.zone_count):
            ...
            self.zones[zid].update_pods(owner_id, pods, enemy, visible, platinum, self.round)
    ...
    def lead_to_unexplored(self, zone):
        ...
        for l in zone.links:
            if not l.is_uptodate(self.round):
                return True
        return False
